...
public class Duck extends RobotPlayer {
	...
	public static void run() throws GameActionException {
		...
		Pair<ArrayList<MapLocation>, Integer> spawnTrapsLocations = getSpawnTrapLocation();
		...
		if(enemyDucksAttack.length > 0 && turnCount >= 200) {
			...
			if(enemyDucksVision.length > 0 && turnCount >= 200 && !rc.hasFlag()) {
				...
				if (turnCount < 200) {
					...
					if(spawnTrapsLocations.value < MAX_SPAWN_TRAPS) {
						for(MapLocation spawnLoc : spawnTrapsLocations.key) {
							if(rc.canBuild(TrapType.EXPLOSIVE, spawnLoc)) {
								rc.build(TrapType.EXPLOSIVE, spawnLoc);
								break;
							}
						}
					}
	...
	// ---------------------------------------------- HELPERS  ------------------------------------------------
	public static Pair<ArrayList<MapLocation>, Integer> getSpawnTrapLocation() throws GameActionException {
		MapLocation[] spawnLocations = rc.getAllySpawnLocations();
		ArrayList<MapLocation> spawnTrapsLocations = new ArrayList<MapLocation>();
		int trapCount = 0;

		for(MapLocation spawnLoc : spawnLocations) {
			if(rc.canSenseLocation(spawnLoc)) {
				if(!rc.senseMapInfo(spawnLoc).getTrapType().equals(TrapType.NONE)) {
					trapCount++;
				}
				spawnTrapsLocations.add(spawn);
			}
		}

		return new Pair<>(spawnTrapsLocations, trapCount);
	}
